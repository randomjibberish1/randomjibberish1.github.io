...
if (array_key_exists('saved_troop', $_POST)) {
    $was_type_saved = false;
    foreach ($all_troops as $typename => $type) {
        foreach ($type as $tier_number => $tier) {
            foreach ($tier as $key => $troop_type) {
                if ($_POST['saved_troop'][$typename][$tier_number][$key] == 1){
                    $_SESSION['saved_troop'][$typename][$tier_number][$key] = true;
                    $was_type_saved = true;
                }
            }
        }
    ...
    if ($was_type_saved == true){
        $_SESSION['troop_type_saved'] = true;
...
//var_dump($_POST);
    ?>
    <center>
        <h1>Hospital Healing Calculator</h1>
    </center>
    <?php
...
// if there are errors, reprint the table with stick values and die.
        if (!empty($errors)) {
            ...
            print_error_table($all_troops, $_POST);
            die;
        }

// calculating resources and totals
        $rss_total = array('troops' => 0, 'stone' => 0, 'wood' => 0, 'ore' => 0, 'food' => 0, 'silver' => 0);
        $rss_calc = array();

        $rss_calc = calculate_resources($all_troops, $rss_calc, $_POST);
        $rss_total = calculate_total($rss_calc, $rss_total, $_POST);


// if there are no troops to heal, state so and die
        if ($rss_total['troops'] == 0) {
            ?>
            <h3> <center>There are no troops to heal!  You're good!</h3>
                ...
                <form action='<?php print $_SERVER['PHP_SELF'] ?>' method='POST'>
                    <table width='600px'>      
                        ...
                        foreach ($all_troops as $typename => $type) {
                            foreach ($type as $tier_number => $tier) {
                                foreach ($tier as $key => $troop_type) {
                                    if ($_POST['quantity'][$typename][$tier_number][$key] != 0) {
                                        if (@$_SESSION['logged_in'] == true) {
                                            ?>
                                            <tr><td><input type='checkbox' name='saved_troop[<?php print $typename ?>][<?php print $tier_number ?>][<?php print $key ?>]' value='1'></td>
                                                <?php
                                            } else {
                                                print '<tr>';
                                            }
                                            printf('<td>%s</td>'    // name of troops
                                                    ...
                                                    . '<td>%s</td>'     // food
                                                    . '<td>%s</td></tr>'     // silver
                                                    , $troop_type['name'], number_format($_POST['quantity'][$typename][$tier_number][$key]), number_format($rss_calc[$typename][$tier_number][$key]['stone']), number_format($rss_calc[$typename][$tier_number][$key]['wood']), number_format($rss_calc[$typename][$tier_number][$key]['ore']), number_format($rss_calc[$typename][$tier_number][$key]['food']), number_format($rss_calc[$typename][$tier_number][$key]['silver']));
                                        }
                                    }
                                }
                            }
                            ...
                            // option to save troop type for the "Saved Options" page
                            print '<tr><td>&nbsp;</td></tr><tr>';
                            if (@$_SESSION['logged_in'] == true) {
                                ?><td colspan='4' align='left'>
                                    <input type='submit' name='save_troops' value='Save Checked Troops'></form></td>
                                    <?php
                                }
...
// request input for troop healing values
    else {
        print_table($all_troops);
    }

// functions
// calculate resource totals
    function calculate_resources($all_troops, $rss_calc, $POST) {
        foreach ($all_troops as $typename => $type) {   // typename is Normal or Strategic
            foreach ($type as $tier_number => $tier) {  // tier_number is Tiers 1-4
                foreach ($tier as $key => $troop_type) {  // key is 0-3 representing Infantry through Siege
                    $rss_calc[$typename][$tier_number][$key]['stone'] = $troop_type['stone'] * $POST['quantity'][$typename][$tier_number][$key];
                    $rss_calc[$typename][$tier_number][$key]['wood'] = $troop_type['wood'] * $POST['quantity'][$typename][$tier_number][$key];
                    $rss_calc[$typename][$tier_number][$key]['ore'] = $troop_type['ore'] * $POST['quantity'][$typename][$tier_number][$key];
                    $rss_calc[$typename][$tier_number][$key]['food'] = $troop_type['food'] * $POST['quantity'][$typename][$tier_number][$key];
                    $rss_calc[$typename][$tier_number][$key]['silver'] = $troop_type['silver'] * $POST['quantity'][$typename][$tier_number][$key];
                }
            }
        }
        return $rss_calc;
        ...
        foreach ($rss_calc as $typename => $type) {   // typename is Normal or Strategic
            foreach ($type as $tier_number => $tier) {  // tier_number is Tiers 1-4
                foreach ($tier as $key => $troop_type) {  // key is 0-3 representing Infantry through Siege
                    ...
                    $rss_total['troops'] += $_POST['quantity'][$typename][$tier_number][$key];
    ...
    function print_table($all_troops) {
        ?>
            <center>
                <table width="800px">
                    <form action="<?php print $_SERVER['PHP_SELF'] ?>" method="POST">
        ...
        foreach ($all_troops as $typename => $type) {
            ...
            if (count($type['Tier 1']) > 3) {
                print "<td colspan=2 align=center width = '16%'><b>Siege</b></td>";
            }
            foreach ($type as $tier_number => $tier) {
                printf('</tr><tr><td><b>%s</b></td>', $tier_number);
                foreach ($tier as $key => $troop_type) {
                    printf("<td align=right><input type=text maxlength=7 size=5 name='quantity[%s][%s][] value=''></td><td>%s</td>", $typename, $tier_number, $troop_type['name']);
                }
    ...
    function print_error_table($all_troops, $POST) {
        ?>
                        <center>
                            <table width="800px">
                                <form action="<?php print $_SERVER['PHP_SELF'] ?>" method="POST">
        ...
        foreach ($all_troops as $typename => $type) {
            ...
            foreach ($type as $tier_number => $tier) {
                ...
                foreach ($tier as $key => $troop_type) {
                    ?> 
                                                <td align=right><input type=text maxlength=7 size=5 name='quantity[<?php print $typename ?>][<?php print $tier_number ?>][]' value='<?php print $POST['quantity'][$typename][$tier_number][$key] ?>'></td><td><?php print $troop_type['name'] ?></td>
                                                <?php
                                            }
                                            print '<tr><td>&nbsp</td></tr>';
                                        }
